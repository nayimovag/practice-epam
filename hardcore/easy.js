...
(async function example() {
  ...
  try {
    ...
    await driver.wait(until.elementIsVisible(driver.findElement(By.xpath('//*[@id="email_quote"]')))).click()
    const originalWindow = await driver.getWindowHandle();
    await driver.switchTo().newWindow('tab');
    await driver.get('https://www.minuteinbox.com/')
    const mailWindow = await driver.getWindowHandle();

    await driver.wait(until.elementIsVisible(driver.findElement(By.xpath("/html/body/div[2]/div[3]/div/a[4]")))).click()
    ...
    await driver.switchTo().frame("myFrame");
    let emailBox = await driver.findElement(By.xpath('//input[@ng-model="emailQuote.user.email"]'))
    await emailBox.sendKeys(Key.chord(Key.CONTROL, 'v'))
    await driver.findElement(By.xpath("//button[@aria-label='Send Email']")).click()
